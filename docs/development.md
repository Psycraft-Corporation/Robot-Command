...
The test project is intentionally not split into many assemblies. Its folders follow behavior: contracts, workflows, MAVLink, simulation, teams, missions, fences, rendering, GUI/CLI, persistence, and infrastructure.
...
an explicitly prerelease tag such as `v0.1.0-beta.8`. A valid tag starts the
`Release` workflow, which validates the source, builds the Windows application
and CLI archives, writes SHA-256 checksums and a release manifest, and creates
a draft GitHub release. A maintainer publishes the draft manually.
...
unsigned for this release phase. Robot Command does not publish NuGet or LinkD;
Azure DevOps remains the publication path for the generated
`Psycraft.Logos.Api.Sdk` package.
