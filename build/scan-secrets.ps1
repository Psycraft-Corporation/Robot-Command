...
if ($SourceOnly) {
    ...
    Get-ChildItem -LiteralPath $root -Recurse -Force -File | ForEach-Object {
        ...
        $isPinnedGStreamerMsi = $relativePath.Replace('\', '/') -ieq $pinnedGStreamerMsi
        $isForbidden = ($parts | Where-Object { $forbiddenNames -contains $_ }) -or
            ...
            (($forbiddenExtensions | Where-Object { $leaf.EndsWith($_, [StringComparison]::OrdinalIgnoreCase) }) -and
                -not $isPinnedGStreamerMsi)
        # The pinned MSI is an approved third-party binary. Keep it in the
        # source tree, but do not interpret its binary payload as text during
        # secret scanning.
        if (-not $isForbidden -and -not $isPinnedGStreamerMsi) {
