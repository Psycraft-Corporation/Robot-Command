...
try {
    if (-not ($appZip.Entries | ForEach-Object { $_.FullName.Replace('\', '/') } | Where-Object { $_ -eq "THIRD-PARTY-NOTICES/GStreamer.txt" })) {
        throw "The Windows application archive does not contain the GStreamer notice."
    }
} finally {
    $appZip.Dispose()
}
