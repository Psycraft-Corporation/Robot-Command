...
$pinnedGStreamerMsi = 'build/third-party/gstreamer/1.26.11/win-x64/gstreamer-1.0-msvc-x86_64-1.26.11.msi'

function Test-IsPinnedGStreamerMsi([string]$RelativePath) {
    return $RelativePath.Replace('\', '/') -ieq $pinnedGStreamerMsi
}
...
if ($SourceOnly) {
    ...
    Get-ChildItem -LiteralPath $root -Recurse -Force -File | ForEach-Object {
        ...
        $isForbidden = ($parts | Where-Object { $forbiddenNames -contains $_ }) -or
            ($leaf -like '.env.*') -or
            (($forbiddenExtensions | Where-Object { $leaf.EndsWith($_, [StringComparison]::OrdinalIgnoreCase) }) -and
                -not (Test-IsPinnedGStreamerMsi $relativePath))
        if (-not $isForbidden) {
...
$violations = foreach ($relativePath in $paths) {
    ...
    if (($parts | Where-Object { $forbiddenNames -contains $_ }) -or
        ...
        (($forbiddenExtensions | Where-Object { $leaf.EndsWith($_, [StringComparison]::OrdinalIgnoreCase) }) -and
            -not (Test-IsPinnedGStreamerMsi $relativePath))) {
        $relativePath
    } elseif (-not (Test-IsPinnedGStreamerMsi $relativePath) -and (Get-Item -LiteralPath $fullPath).Length -gt 25MB) {
...
$contentViolations = foreach ($relativePath in $paths) {
    if (Test-IsPinnedGStreamerMsi $relativePath) {
        continue
    }
