...
$appArchive = [IO.Compression.ZipFile]::OpenRead((Join-Path $directory "RobotCommand-$Version-$Runtime.zip"))
try {
    $gstreamerIncluded = @($appArchive.Entries | Where-Object {
        $_.FullName.Replace('\', '/') -eq 'native/gstreamer/bin/gst-launch-1.0.exe'
    }).Count -gt 0
}
finally {
    $appArchive.Dispose()
...
$manifest = [ordered]@{
    ...
    gstreamerIncluded = $gstreamerIncluded
    artifacts = $artifacts
}
