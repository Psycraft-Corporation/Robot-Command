...
internal static class SessionCommands
{
    private const string HelpText = """
        ...
        UNITS AND CONTROL
          ...
          manual profile list|show|create|select|update|delete ...
            ...
            switch (group)
            {
                ...
                case "map":
                    if (args.Positionals.FirstOrDefault()?.Equals("package", StringComparison.OrdinalIgnoreCase) == true)
                        return await WorkspaceCommands.ExecuteAsync("map", args, evidence, media, mapLibrary, teamObserver, preferences, reporter, cancellationToken);
                ...
                case "manual":
                    return await ManualAsync(args, manual, reporter, watcher, manualOwner, manualOwnerId, cancellationToken);
                case "operator":
                ...
                case "team":
                    ...
                    return await WorkspaceCommands.ExecuteAsync(group, args, evidence, media, mapLibrary, teamObserver, preferences, reporter, cancellationToken);
                case "mission" or "flight-mission" or "task" or "behaviour" or "geometry" or "autonomy" or "px4" or "ardupilot" or "fence" or "px4-fence" or "sik" or "plan":
                    return await WorkflowCommands.ExecuteAsync(group, args, autonomy, behaviours, geometry, flightMissions, fences, parameters, sik, reviewed, reporter, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        switch (command)
        {
            ...
            case "profile":
                ...
                if (profileCommand == "list") reporter.Event("manual.profiles", manual.Profiles);
                else if (profileCommand == "show")
                {
                    var profileId = args.Positionals.Count > 2 ? args.Positionals[2] : manual.ActiveProfileId;
                    reporter.Event("manual.profile", manual.Profiles.FirstOrDefault(item => item.Id == profileId) ?? throw new KeyNotFoundException("Manual-control profile was not found."));
                }
                else if (profileCommand == "create") reporter.Event("manual.profile.created", await manual.CreateProfileAsync(args.Required("name"), cancellationToken: cancellationToken));
                else if (profileCommand == "select") reporter.Event("manual.profile.selected", await manual.SelectProfileAsync(RequirePosition(args, 2, "A profile ID is required."), cancellationToken));
                else if (profileCommand == "delete")
                {
                    var profileId = RequirePosition(args, 2, "A profile ID is required.");
                    await manual.DeleteProfileAsync(profileId, cancellationToken);
                    reporter.Event("manual.profile.deleted", new { Id = profileId });
                }
                else if (profileCommand == "update")
                {
                    var profileId = RequirePosition(args, 2, "A profile ID is required.");
                    var current = manual.Profiles.FirstOrDefault(item => item.Id == profileId)?.Settings ?? throw new KeyNotFoundException("Manual-control profile was not found.");
                    var updated = current with
                    {
                        DeadZone = OptionalNumber(args, "dead-zone") ?? current.DeadZone,
                        Expo = OptionalNumber(args, "expo") ?? current.Expo,
                        MaximumHorizontalSpeedMetresPerSecond = OptionalNumber(args, "horizontal-speed") ?? current.MaximumHorizontalSpeedMetresPerSecond,
                        MaximumVerticalSpeedMetresPerSecond = OptionalNumber(args, "vertical-speed") ?? current.MaximumVerticalSpeedMetresPerSecond,
                        MaximumYawRateDegreesPerSecond = OptionalNumber(args, "yaw-rate") ?? current.MaximumYawRateDegreesPerSecond,
                        TakeoffAltitudeAglMetres = OptionalNumber(args, "takeoff-agl") ?? current.TakeoffAltitudeAglMetres
                    };
                    reporter.Event("manual.profile.updated", await manual.UpdateProfileAsync(profileId, args.Get("name"), updated, cancellationToken));
                }
                ...
                else throw new ArgumentException("Profile command must be list, show, create, select, update, delete, or set.");
                break;
