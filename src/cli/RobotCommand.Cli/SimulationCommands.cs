            ...
            while (!stopping.IsCancellationRequested && DateTimeOffset.UtcNow - started < TimeSpan.FromSeconds(seconds))
            {
                ...
                if (worker.LatestSnapshot is { } snapshot)
                {
                    if (json)
                        ...
                        Console.WriteLine(CliHumanFormatter.Format("simulation.worker", new
                        {
                            snapshot.Sequence,
                            Ghosts = snapshot.Ghosts.Count,
                            snapshot.HeartbeatAt,
                            PhysicsTicks = snapshot.Metrics?.PhysicsTicks ?? 0,
                            MissedDeadlines = snapshot.Metrics?.MissedDeadlines ?? 0,
                            P95TickMilliseconds = snapshot.Metrics?.PhysicsP95Milliseconds ?? 0,
                            MaxTickMilliseconds = snapshot.Metrics?.MaxTickMilliseconds ?? 0,
                            SnapshotPublications = snapshot.Metrics?.SnapshotPublications ?? 0
                        }));
                }
