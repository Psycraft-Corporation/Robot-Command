...
/// Thrustmaster T.16000M. It intentionally reports raw axis/button capacity so
...
public sealed class WindowsRawJoystickInputDeviceProvider : IManualInputDeviceProvider, IHostedService
{
    ...
    {
        var name = controller.HardwareVendorId == 0x044F && controller.HardwareProductId == 0xB10A
            ? "Thrustmaster T.16000M"
            : string.IsNullOrWhiteSpace(controller.DisplayName)
                ? $"Joystick {controller.HardwareVendorId:X4}:{controller.HardwareProductId:X4}"
                : controller.DisplayName;
        return new ManualInputDevice(id, name, true, ManualInputDeviceKind.Joystick, controller.AxisCount, controller.ButtonCount);
    }
