...
/// </summary>
public sealed class ManualControlService : IManualControlService, IHostedService, IDisposable
{
    ...
    private ManualControlProfileLibrary _profileLibrary = ManualControlProfileLibrary.Default;
    ...
    public IReadOnlyList<ManualControlProfileRecord> Profiles => _profileLibrary.Profiles;
    public string ActiveProfileId => _profileLibrary.ActiveProfileId;
    ...
    {
        _profileLibrary = await _profileStore.LoadLibraryAsync(cancellationToken);
        _profile = _profileLibrary.Profiles.FirstOrDefault(item => item.Id == _profileLibrary.ActiveProfileId)?.Settings.Normalize() ?? new ManualControlProfile();
        _selectedDeviceId ??= Devices.Count > 0 ? Devices[0].Id : null;
    ...
    public Task SetSelectedDeviceAsync(string? deviceId, CancellationToken cancellationToken = default)
    {
        ...
        _selectedDeviceId = Devices.Any(device => device.Id == deviceId) ? deviceId : Devices.Count > 0 ? Devices[0].Id : null;
        PublishChanged();
        return Task.CompletedTask;
    }
    ...
    public async Task SaveProfileAsync(ManualControlProfile profile, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            EnsureProfileChangesAllowed();
            _profile = profile.Normalize();
            _profileLibrary = _profileLibrary with
            {
                Profiles = _profileLibrary.Profiles
                    .Select(item => item.Id == _profileLibrary.ActiveProfileId ? item with { Settings = _profile } : item)
                    .ToArray()
            };
            await _profileStore.SaveLibraryAsync(_profileLibrary, cancellationToken);
            PublishChanged();
        }
        finally
        {
    ...
    public async Task<ManualControlProfileRecord> CreateProfileAsync(string name, ManualControlProfile? profile = null, CancellationToken cancellationToken = default)
    {
        await _transitionGate.WaitAsync(cancellationToken);
        try
        {
            EnsureProfileChangesAllowed();
            var normalizedName = ValidateProfileName(name);
            if (_profileLibrary.Profiles.Any(item => string.Equals(item.Name, normalizedName, StringComparison.OrdinalIgnoreCase)))
                throw new InvalidOperationException($"A manual-control profile named '{normalizedName}' already exists.");
            var record = new ManualControlProfileRecord($"manual-{Guid.NewGuid():N}", normalizedName, (profile ?? new ManualControlProfile()).Normalize());
            _profileLibrary = _profileLibrary with { Profiles = [.. _profileLibrary.Profiles, record] };
            await _profileStore.SaveLibraryAsync(_profileLibrary, cancellationToken);
            PublishChanged();
            return record;
        }
    ...
    public async Task<ManualControlProfileRecord> SelectProfileAsync(string profileId, CancellationToken cancellationToken = default)
    {
        await _transitionGate.WaitAsync(cancellationToken);
        try
        {
            EnsureProfileChangesAllowed();
            var record = FindProfile(profileId);
            _profileLibrary = _profileLibrary with { ActiveProfileId = record.Id };
            _profile = record.Settings.Normalize();
            await _profileStore.SaveLibraryAsync(_profileLibrary, cancellationToken);
            PublishChanged();
            return record with { Settings = _profile };
        }
        finally { _transitionGate.Release(); }
    }

    public async Task<ManualControlProfileRecord> UpdateProfileAsync(string profileId, string? name, ManualControlProfile? profile, CancellationToken cancellationToken = default)
    {
        await _transitionGate.WaitAsync(cancellationToken);
        try
        {
            EnsureProfileChangesAllowed();
            var existing = FindProfile(profileId);
            var normalizedName = name is null ? existing.Name : ValidateProfileName(name);
            if (_profileLibrary.Profiles.Any(item => item.Id != existing.Id && string.Equals(item.Name, normalizedName, StringComparison.OrdinalIgnoreCase)))
                throw new InvalidOperationException($"A manual-control profile named '{normalizedName}' already exists.");
            var updated = existing with { Name = normalizedName, Settings = (profile ?? existing.Settings).Normalize() };
            _profileLibrary = _profileLibrary with { Profiles = _profileLibrary.Profiles.Select(item => item.Id == updated.Id ? updated : item).ToArray() };
            if (updated.Id == _profileLibrary.ActiveProfileId) _profile = updated.Settings;
            await _profileStore.SaveLibraryAsync(_profileLibrary, cancellationToken);
            PublishChanged();
            return updated;
        }
    ...
    public async Task DeleteProfileAsync(string profileId, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            EnsureProfileChangesAllowed();
            var existing = FindProfile(profileId);
            if (_profileLibrary.Profiles.Count <= 1)
                throw new InvalidOperationException("At least one manual-control profile must remain.");
            var wasActive = existing.Id == _profileLibrary.ActiveProfileId;
            var remaining = _profileLibrary.Profiles.Where(item => item.Id != existing.Id).ToArray();
            var activeId = existing.Id == _profileLibrary.ActiveProfileId ? remaining[0].Id : _profileLibrary.ActiveProfileId;
            _profileLibrary = _profileLibrary with { ActiveProfileId = activeId, Profiles = remaining };
            if (wasActive)
                _profile = remaining.First(item => item.Id == activeId).Settings.Normalize();
            await _profileStore.SaveLibraryAsync(_profileLibrary, cancellationToken);
            PublishChanged();
        }
        finally { _transitionGate.Release(); }
    ...
    private void EnsureProfileChangesAllowed()
    {
        if (_snapshot.IsActive || _snapshot.State is ManualControlSessionState.Acquiring or ManualControlSessionState.Releasing)
            throw new InvalidOperationException("Change the manual-control profile after releasing control.");
    }

    private ManualControlProfileRecord FindProfile(string profileId)
        => _profileLibrary.Profiles.FirstOrDefault(item => string.Equals(item.Id, profileId, StringComparison.Ordinal))
            ?? throw new KeyNotFoundException("Manual-control profile was not found.");

    private static string ValidateProfileName(string name)
    {
        var normalized = name.Trim();
        if (normalized.Length is < 1 or > 80) throw new ArgumentException("Profile name must be between 1 and 80 characters.", nameof(name));
        return normalized;
    }
    ...
    private async Task<bool> TakeControlCoreAsync(string vehicleId, CancellationToken cancellationToken)
    {
        ...
        var deviceId = _selectedDeviceId ?? (Devices.Count > 0 ? Devices[0].Id : null);
        if (deviceId is null || !_devices.TryGetReading(deviceId, _profile.MappingFor(deviceId), out var reading))
        {
            SetSnapshot(ManualControlSessionSnapshot.Empty with { State = ManualControlSessionState.Blocked, Status = "Connect a supported gamepad or joystick first." });
            return false;
        }
        ...
        await _dispatcher.InvokeAsync(() => _commands.Upsert(new OperationalCommandRecord(
            _sessionCommandId, "ManualControl", "Vehicle", vehicle.Id, vehicle.ConnectionIds.Count > 0 ? vehicle.ConnectionIds[0] : null,
        ...
        var sessionSnapshot = new ManualControlSessionSnapshot(ManualControlSessionState.Hold, backend, vehicle.Id, vehicle.Name, deviceId,
            Devices.FirstOrDefault(item => item.Id == deviceId)?.Name ?? "Controller", now, reading.Timestamp,
    ...
    private async Task QueueDiscreteAsync(OperatorCommandKind command, string button, CancellationToken cancellationToken)
    {
        ...
        var plan = await _registry.RunManualSubmissionAsync(() =>
            _operations.PrepareAsync(snapshot.TargetVehicleId, command, "Controller", parameters, cancellationToken));
        if (!plan.CanSubmit)
        {
    ...
    private void OnDevicesChanged(object? sender, EventArgs e) => PublishChanged();
