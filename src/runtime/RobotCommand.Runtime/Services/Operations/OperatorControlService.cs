...
public sealed class OperatorControlService : IOperatorControlService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var normalizedParameters = command is OperatorCommandKind.Disarm or
            ...
            findings.All(item => item.Severity != OperatorPreflightSeverity.Blocking))
        {
            var preparation = await _gateway.PrepareAsync(
                new OperatorCommandRequest(
                    ...
                    target,
                    normalizedReason,
                    IsEmergencyDispatch(command, normalizedParameters, telemetry),
                    now,
    ...
    private static bool IsEmergencyDispatch(
        OperatorCommandKind command,
        OperatorCommandParameters? parameters,
        VehicleTelemetryRecord? telemetry)
        => command == OperatorCommandKind.Disarm
            ? parameters?.AirborneDisarmConfirmed == true
            : OperatorControlRules.SafetyFor(command, telemetry) == OperatorCommandSafety.Critical;

    private static bool IsEmergencyDispatch(OperatorCommandPlan plan)
        => plan.Command == OperatorCommandKind.Disarm
            ? plan.Parameters?.AirborneDisarmConfirmed == true
            : plan.Emergency;
