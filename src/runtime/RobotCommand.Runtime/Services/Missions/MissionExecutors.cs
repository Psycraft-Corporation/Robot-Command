...
public sealed class Px4MissionExecutor(IMavlinkConnectionRegistry connections) : IFlightMissionExecutor
{
    ...
    {
        var result = await Client(connectionId).SetMissionModeAsync(vehicleId, paused, token, restartFromBeginning);
...
public sealed class ArduPilotMissionExecutor(IMavlinkConnectionRegistry connections) : IFlightMissionExecutor
{
    ...
    {
        var result = await Client(connectionId).SetMissionModeAsync(vehicleId, paused, cancellationToken, restartFromBeginning, sendMissionStartCommand);
