...
public sealed class LogosConnectionManager : ILogosConnectionManager, IDisposable
{
    ...
    // An explicit Disconnect is an operator decision. Keep it separate from
    // the transport's connect-request flag so supervision cannot immediately
    // undo that decision while auto-reconnect remains enabled.
    private readonly HashSet<string> _explicitlyDisconnected = new(StringComparer.Ordinal);
    ...
    public async Task RemoveAsync(string connectionId, CancellationToken cancellationToken = default)
    {
        ...
        lock (_gate)
        {
            ...
            _explicitlyDisconnected.Remove(connectionId);
    ...
    public async Task DisconnectAsync(string connectionId, CancellationToken cancellationToken = default)
    {
        ...
        lock (_gate)
        {
            _explicitlyDisconnected.Add(connectionId);
        }
        await connection.DisconnectAsync(cancellationToken);
        await PublishAllAsync(cancellationToken);
    }
    ...
    {
        foreach (var connection in SnapshotConnections().Where(item => item.Definition.AutoConnect || item.Definition.AutoReconnect))
        {
            cancellationToken.ThrowIfCancellationRequested();
            lock (_gate)
            {
                // Starting the application is an explicit new session. A
                // suppression from a previous session must not survive it.
                _explicitlyDisconnected.Remove(connection.Definition.Id);
            }
            try
            {
    ...
    public async Task SuperviseAsync(CancellationToken cancellationToken = default)
    {
        ...
        foreach (var connection in SnapshotConnections())
        {
            ...
            lock (_gate)
            {
                if (_explicitlyDisconnected.Contains(connection.Definition.Id))
                {
                    continue;
                }
            }
