...
public sealed class FormationAuthoringWorkflow : IFormationAuthoringWorkflow, IDisposable
{
    ...
    public async Task<FormationWorkflowSnapshot> CreateAsync(FormationCreateRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var document = new FormationDocument(FormationDocument.CurrentSchemaVersion, CreateId(name), name, now, now,
            []);
        return Map(await _store.SaveAsync(document, allowEmpty: true, cancellationToken: cancellationToken));
    }
    public async Task<FormationWorkflowSnapshot> SaveAsync(FormationWorkflowSnapshot formation, CancellationToken cancellationToken = default)
    {
        if (!_store.TryGet(formation.Id, out var existing) || existing is null) throw new KeyNotFoundException("Formation was not found.");
        EnsureUniqueName(formation.Name, formation.Id);
        var document = new FormationDocument(FormationDocument.CurrentSchemaVersion, formation.Id, RequireName(formation.Name), existing.CreatedAt,
            formation.UpdatedAt, formation.Members.Select(item => new FormationMember(item.Id, item.Name, item.EastMetres, item.UpMetres, item.NorthMetres)).ToArray());
        return Map(await _store.SaveAsync(document, cancellationToken: cancellationToken));
    }
    ...
    private async Task<FormationWorkflowSnapshot> MutateAsync(string id, Func<FormationDocument, FormationDocument> mutate, CancellationToken token)
    {
        ...
        return Map(await _store.SaveAsync(mutate(document), cancellationToken: token));
    }
