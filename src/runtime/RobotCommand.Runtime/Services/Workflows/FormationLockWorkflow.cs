...
public sealed class FormationLockWorkflow : IFormationLockWorkflow, IHostedService, IAsyncDisposable
{
    ...
    private bool Advance(FormationState state, double elapsedSeconds)
    {
        var changed = false;
    ...
    private void BeginManeuverLocked(FormationState state, string maneuverName)
    {
    ...
    private IReadOnlyList<PendingFormationTarget> CaptureTargetBatch(FormationState state, IReadOnlySet<string>? onlyMembers, bool begin)
    {
        lock (_gate)
        {
            if (!ReferenceEquals(_state, state)) return [];
    ...
    private IReadOnlyList<WorkflowFinding> ValidateMove(string teamId, double latitudeDegrees, double longitudeDegrees)
    {
    ...
    private IReadOnlyList<WorkflowFinding> ValidateAltitude(string teamId, double altitudeAglMetres)
    {
        var findings = new List<WorkflowFinding>();
        if (!double.IsFinite(altitudeAglMetres) || altitudeAglMetres < 0)
    ...
    private IReadOnlyList<WorkflowFinding> ValidateTransform(string teamId, double value, bool isScale)
    {
        var findings = new List<WorkflowFinding>();
