...
public sealed class SelectionWorkflow : ISelectionWorkflow
{
    ...
    public SelectionWorkflowSnapshot Current => new(
        _selection.Current.Kind == SelectionKind.Vehicle ? _selection.Current.Id : _selection.SelectedUnitIds.Count > 0 ? _selection.SelectedUnitIds[0] : null,
...
public sealed class MapViewWorkflow : IMapViewWorkflow, IDisposable
{
    ...
    public async Task ApplySavedViewAsync(string id, CancellationToken cancellationToken = default)
    {
        ...
        await _selection.SetUnitsAsync(selected, view.UnitSelectionAnchorId ?? (selected.Count > 0 ? selected[0] : null), cancellationToken);
        PublishChanged();
    }
    ...
    public void Dispose() => _changeNotifier.Dispose();
}

/// <summary>Projects the current Runtime map presentation without exposing rendering objects.</summary>
public sealed class MapSceneObservationWorkflow : IMapSceneObservationWorkflow, IDisposable
{
...
public sealed class GhostUnitWorkflow : IGhostUnitWorkflow
{
    ...
    {
        if (request.Count is < 1 or > 50) throw new ArgumentOutOfRangeException(nameof(request), "Create between 1 and 50 Ghosts.");
        ...
        for (var index = 0; index < request.Count; index++)
        {
            ...
            var spawnViewport = GhostSpawnLayout.ForBatch(viewport, index, request.Count);
            var vehicle = await _ghosts.CreateAsync(request.ProfileId, spawnViewport, request.HeadingDegrees, cancellationToken);
    ...
    private GhostUnitSnapshot? Project(string unitId)
    {
        ...
        return new(unitId, vehicle.Name, vehicle.ConnectionIds.Count > 0 ? vehicle.ConnectionIds[0] : string.Empty, latitude, longitude, telemetry.AltitudeAglMetres ?? 0, telemetry.HeadingDegrees ?? 90, telemetry.Armed && !string.Equals(telemetry.LandedState, "Landed", StringComparison.OrdinalIgnoreCase) ? "Flying" : telemetry.Armed ? "Armed" : "Disarmed", null)
        {
...
public sealed class ManualControlWorkflow : IManualControlWorkflow
{
    ...
    public event EventHandler? Changed;
    public IReadOnlyList<ManualInputDeviceSnapshot> Devices => _manual.Devices.Select((item, index) => new ManualInputDeviceSnapshot(item.Id, GenericDeviceName(_manual.Devices, item, index), item.IsConnected, item.Id == _manual.SelectedDeviceId, GenericDeviceKind(item.Kind), item.AxisCount, item.ButtonCount)).ToArray();
    public ManualControlProfileSnapshot Profile => ToSnapshot(_manual.Profile);
    public IReadOnlyList<ManualControlProfileEntrySnapshot> Profiles => _manual.Profiles.Select(item => new ManualControlProfileEntrySnapshot(item.Id, item.Name, ToSnapshot(item.Settings), item.Id == _manual.ActiveProfileId)).ToArray();
    public string ActiveProfileId => _manual.ActiveProfileId;
    ...
    public async Task<ManualControlProfileEntrySnapshot> CreateProfileAsync(string name, ManualControlProfileSnapshot? settings = null, CancellationToken cancellationToken = default)
        => ToEntry(await _manual.CreateProfileAsync(name, settings is null ? null : ToModel(settings), cancellationToken), _manual.ActiveProfileId);
    public async Task<ManualControlProfileEntrySnapshot> SelectProfileAsync(string profileId, CancellationToken cancellationToken = default)
        => ToEntry(await _manual.SelectProfileAsync(profileId, cancellationToken), _manual.ActiveProfileId);
    public async Task<ManualControlProfileEntrySnapshot> UpdateProfileAsync(string profileId, string? name, ManualControlProfileSnapshot? settings, CancellationToken cancellationToken = default)
        => ToEntry(await _manual.UpdateProfileAsync(profileId, name, settings is null ? null : ToModel(settings), cancellationToken), _manual.ActiveProfileId);
    public Task DeleteProfileAsync(string profileId, CancellationToken cancellationToken = default) => _manual.DeleteProfileAsync(profileId, cancellationToken);
    ...
    private static ManualControlProfileEntrySnapshot ToEntry(ManualControlProfileRecord value, string activeProfileId)
        => new(value.Id, value.Name, ToSnapshot(value.Settings), value.Id == activeProfileId);

    private static string GenericDeviceKind(ManualInputDeviceKind kind)
        => kind == ManualInputDeviceKind.Joystick ? "Joystick" : "Gamepad";

    private static string GenericDeviceName(IReadOnlyList<ManualInputDevice> devices, ManualInputDevice device, int index)
    {
        var ordinal = 1;
        for (var i = 0; i < index; i++)
        {
            if (devices[i].Kind == device.Kind)
                ordinal++;
        }

        return $"{GenericDeviceKind(device.Kind)} {ordinal}";
    }
