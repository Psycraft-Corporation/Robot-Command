...
public static class OnlineMapTileSources
{
    ...
    public static HttpTileSource CreateWeather(WeatherRadarSnapshot snapshot)
    {
        ArgumentNullException.ThrowIfNull(snapshot);
        if (!snapshot.HasFrame)
        {
            throw new ArgumentException("A usable weather radar frame is required.", nameof(snapshot));
        }

        var cache = Caches.GetOrAdd("weather", CreateCache);
        return new HttpTileSource(
            new GlobalSphericalMercator(0, snapshot.MaximumZoom),
            snapshot.TileUrlTemplate!,
            [],
    ...
    public static string CacheDirectory(string styleId)
    {
        ...
        return Path.Combine(
            ...
            "Robot Command",
            "MapTileCache",
            string.Equals(styleId, "weather", StringComparison.OrdinalIgnoreCase)
                ? "weather"
