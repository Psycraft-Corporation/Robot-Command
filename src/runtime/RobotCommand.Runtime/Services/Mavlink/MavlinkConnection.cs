...
public sealed class MavlinkConnection : IManagedConnection, IMavlinkParameterClient, IMavlinkMissionClient
{
    ...
    private const uint ArduPilotGuidedCustomMode = 4;
    ...
    private readonly IReadOnlyDictionary<string, IVehicleDiagnosticsProvider> _diagnosticsProviders;
    ...
    public LogosConnectionObservation? LastObservation => Observations.FirstOrDefault();
            ...
            while (!timeout.IsCancellationRequested)
            {
                ...
                if (signal.Kind == MissionSignalKind.Ack)
                {
                    ...
                    return new(signal.Result == 0, signal.Result == 0
                        ...
                        : $"Vehicle rejected mission item {session.LastRequestedSequence?.ToString() ?? "unknown"}: {DescribeMissionResult(signal.Result)}.", items);
                }
            }
    ...
    public async Task<MavlinkMissionTransferResult> SetMissionModeAsync(string vehicleId, bool paused, CancellationToken cancellationToken = default, bool restartFromBeginning = true, bool sendMissionStartCommand = false)
    {
        if (!TryGetVehicle(vehicleId, out var systemId, out var componentId, out _, out var adapter) || adapter is null || !adapter.SupportsMissionExecution)
            return new(false, "The selected unit is not an available PX4 or ArduPilot MAVLink multicopter.", []);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // ArduCopter leaves a landed/disarmed vehicle in Land after an
        // explicit Land command.  Land is a terminal flight mode and ArduPilot
        // correctly rejects ARM from it, even though the vehicle is otherwise
        // ready.  QGC moves the vehicle back to an armable manual mode before
        // arming.  Do the same only as part of an explicit Arm request; this
        ...
        if (adapter.Profile == MavlinkAutopilotProfile.ArduPilot &&
            ...
            !telemetry.Armed &&
            IsLandedTelemetry(telemetry.LandedState) &&
            !IsArduPilotArmableMode(telemetry.AirframeMode))
        {
            var requestedAt = DateTimeOffset.UtcNow;
            var mode = await SendModeAsync(
                systemId,
                MavlinkValues.ArduPilotStabilizeCustomMode,
                "ArduPilot Stabilize before Arm",
                cancellationToken);
            if (!mode.Accepted ||
                    ...
                    cancellationToken))
            {
                return MavlinkCommandDispatchResult.Rejected(
                    $"ArduPilot could not leave {telemetry.AirframeMode} mode before Arm. " +
                    "The vehicle must confirm Stabilize mode before it can be armed.");
            }
        }
            ...
            !string.Equals(telemetry.AirframeMode, "Guided", StringComparison.OrdinalIgnoreCase))
        {
            ...
            var mode = await SendModeAsync(
                systemId,
                ArduPilotGuidedCustomMode,
                "ArduPilot Guided",
                cancellationToken);
            if (!mode.Accepted || !await WaitForModeAsync(systemId, ArduPilotGuidedCustomMode, requestedAt, cancellationToken))
            {
                return MavlinkCommandDispatchResult.Rejected(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var requestedAt = DateTimeOffset.UtcNow;
            var mode = await SendModeAsync(systemId, ArduPilotGuidedCustomMode, "ArduPilot Guided formation", cancellationToken);
            if (!mode.Accepted || !await WaitForModeAsync(systemId, ArduPilotGuidedCustomMode, requestedAt, cancellationToken))
            {
                await StopArduPilotFormationControlAsync(vehicleId, lockId,
    ...
    private void ProcessMissionPacket(MavlinkPacket packet)
    {
        ...
        {
            _logger.LogDebug("Ignoring MAVLink mission message {MessageId} with mission type {MissionType}; transfer expects {ExpectedMissionType}.", packet.MessageId, packetMissionType, session.MissionType);
            return;
        }
        _logger.LogInformation(
        ...
        switch (packet.MessageId)
        {
            ...
            case MavlinkMessageIds.MissionAck:
                _logger.LogInformation("MAVLink mission transfer received MAV_MISSION_RESULT {Result}.", packet.Byte("type"));
                session.Set(new(MissionSignalKind.Ack, 0, packet.Byte("type")));
                break;
        ...
        CancellationToken cancellationToken)
    {
        ...
        lock (_gate)
        {
            if (_systems.TryGetValue(systemId, out var system) &&
                ...
                mode is not ("Stabilize" or "Acro" or "AltHold" or "Sport" or "Loiter" or "PosHold"))
            {
    ...
    private void ProcessStatusText(SystemState system, MavlinkPacket packet)
    {
        ...
        if (text.StartsWith("Preflight Fail:", StringComparison.OrdinalIgnoreCase) ||
            text.StartsWith("Arming denied:", StringComparison.OrdinalIgnoreCase) ||
            (IsArduPilotSystem(system) && text.StartsWith("PreArm:", StringComparison.OrdinalIgnoreCase)))
        {
    ...
    private async Task RunArduPilotManualPumpAsync(byte systemId, ArduPilotManualSession session)
    {
        ...
        {
            session.Failure = ex.Message;
            _logger.LogWarning(ex, "ArduPilot manual-input stream failed for {ConnectionId}/{SystemId}", Definition.Id, systemId);
        }
    }
    ...
    private static bool IsLandedTelemetry(string? landedState)
        => landedState?.Contains("land", StringComparison.OrdinalIgnoreCase) == true ||
           landedState?.Contains("ground", StringComparison.OrdinalIgnoreCase) == true;

    private static bool IsArduPilotArmableMode(string? mode)
        => mode is not null &&
           (mode.Equals("Stabilize", StringComparison.OrdinalIgnoreCase) ||
            mode.Equals("Acro", StringComparison.OrdinalIgnoreCase) ||
            mode.Equals("AltHold", StringComparison.OrdinalIgnoreCase) ||
            mode.Equals("Sport", StringComparison.OrdinalIgnoreCase) ||
            mode.Equals("Loiter", StringComparison.OrdinalIgnoreCase) ||
            mode.Equals("PosHold", StringComparison.OrdinalIgnoreCase));
    ...
    private static string FormatCounter(ushort? value) => value is null or ushort.MaxValue ? "not reported" : value.Value.ToString();
    ...
    private async Task TransitionToHoldAfterCompletionAsync(byte systemId, byte componentId)
    {
        ...
        {
            _logger.LogWarning(ex, "Could not place {Autopilot} system {SystemId} into Hold", ConfiguredAutopilotName, systemId);
        }
    }
    ...
    private void TrackSequence(SystemState system, MavlinkPacket packet)
    {
        ...
        string? subjectId = null)
    {
        ...
        {
            case "Error":
                _logger.LogError("{MavlinkAuditMessage}", auditMessage);
                break;
            case "Warning":
                _logger.LogWarning("{MavlinkAuditMessage}", auditMessage);
                break;
            default:
                _logger.LogInformation("{MavlinkAuditMessage}", auditMessage);
                break;
        }
    }
