...
public sealed class GhostUnitService : IGhostUnitService, IHostedService
{
    ...
    // A delayed timer callback must not make simulated time run slower than
    // wall-clock time. Catch up a bounded number of fixed steps while keeping
    // the simulation loop responsive when the host is heavily loaded.
    ...
    public bool TryGetMissionProgress(string vehicleId, out FlightMissionExecutorProgress progress)
    {
        lock (_gate)
        {
            ...
            var awaitingDecision = mission.State == FlightMissionExecutionState.Completed && mission.Artifact.Items.Count > 0 && mission.Artifact.Items[^1].Kind == FlightMissionStepKind.Land;
            ...
            while (await timer.WaitForNextTickAsync(cancellationToken))
            {
                try
                {
                    ...
                    accumulatedSeconds = Math.Min(
                        accumulatedSeconds + Math.Clamp(elapsedSeconds, 0d, MaximumCatchUpSteps * TickSeconds),
                        MaximumCatchUpSteps * TickSeconds);
                    ...
                    accumulatedSeconds -= stepCount * TickSeconds;
                    var changed = false;
                    lock (_gate)
                    {
    ...
    private static bool StepMission(GhostState ghost, GhostMissionRuntime mission)
    {
        ...
        bool landed)
    {
        ...
        public int Number { get; } = int.Parse(vehicleId[6..], CultureInfo.InvariantCulture);
