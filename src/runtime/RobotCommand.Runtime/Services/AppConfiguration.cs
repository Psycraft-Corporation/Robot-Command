...
public sealed class AppConfiguration
{
    ...
    public string? MediaMtxBinPath { get; init; }

    public VideoProtocolPreference DefaultVideoProtocol { get; init; } = VideoProtocolPreference.Automatic;
    ...
    public static AppConfiguration Load(string baseDirectory)
    {
        ...
        return new AppConfiguration
        {
            ...
            MediaMtxBinPath = ResolveOptionalPath(baseDirectory, config.MediaMtxBinPath),
            DefaultVideoProtocol = config.DefaultVideoProtocol,
    ...
    private static void Merge(MutableConfiguration config, string path)
    {
        ...
        if (root.TryGetProperty("video", out var video) && video.ValueKind == JsonValueKind.Object)
        {
            ...
            config.MediaMtxBinPath = ReadString(video, "mediaMtxBinPath") ?? config.MediaMtxBinPath;
            config.DefaultVideoProtocol = ParseVideoProtocol(
                ReadString(video, "defaultProtocol"),
                config.DefaultVideoProtocol);
    ...
    private static VideoProtocolPreference ParseVideoProtocol(
        string? value,
        VideoProtocolPreference fallback)
        => value?.Trim().ToLowerInvariant() switch
        {
            "automatic" or "auto" => VideoProtocolPreference.Automatic,
            "rtsp" => VideoProtocolPreference.Rtsp,
            "hls" => VideoProtocolPreference.Hls,
            "webrtc" or "whep" => VideoProtocolPreference.WebRtc,
            _ => fallback
        };
    ...
    private sealed class MutableConfiguration
    {
        ...
        public string? MediaMtxBinPath { get; set; }

        public VideoProtocolPreference DefaultVideoProtocol { get; set; } = VideoProtocolPreference.Automatic;
