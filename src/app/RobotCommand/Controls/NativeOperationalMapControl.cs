...
public sealed class NativeOperationalMapControl : Grid
{
    ...
    private static IReadOnlyList<IFeature> CreateGeometryFeatures(
    ...
    private void UpdateWaypointDirectionArrows(
        MapGeometryVisual geometry,
        Viewport viewport,
        IReadOnlyList<AvaloniaPolyline> arrows)
    {
        foreach (var arrow in arrows)
        {
            arrow.IsVisible = false;
    ...
    private bool TryGetMapCommandTarget(MPoint world, out MapCommandTarget target)
    {
        target = default!;
