...
public sealed partial class GeometryWorkspaceViewModel : ObservableObject
{
        ...
        get
        {
            ...
            if (SelectedRemote is { } remote)
            {
                ...
                return string.Join(Environment.NewLine,
                    ...
                    $"Updated: {record.UpdatedAt?.ToLocalTime().ToString("g") ?? "Not reported"}");
            }
    ...
    public string RegistryWatchDetails
    {
        get
        {
            ...
            var lastMessage = watch.LastMessageAt?.ToLocalTime().ToString("g") ?? "No message received";
    ...
    {
        var minimum = policy.MinimumAltitudeMetres?.ToString("0.##") ?? "unbounded";
        var maximum = policy.MaximumAltitudeMetres?.ToString("0.##") ?? "unbounded";
