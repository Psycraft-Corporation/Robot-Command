...
public sealed class MapLibraryViewModel : ObservableObject
{
        ...
        ILocalizationService? localization = null)
    {
        ...
        _statusMessage = _localization.Get("MapReady");
        Packages = [];
    ...
    public string ActivePackageText => _catalog.ActivePackage is { } active
        ...
        : _localization.Get("MapNoActivePackage");
    ...
    public string SelectedPackageTitle => SelectedPackage?.DisplayName ?? _localization.Get("MapNoPackageSelected");
        ...
        get
        {
            ...
            return string.Join(Environment.NewLine,
                ...
                $"Size: {FormatBytes(package.InstalledSizeBytes)}",
                 $"Dataset: {package.DatasetDate?.ToLocalTime().ToString("d", CultureInfo.CurrentCulture) ?? "Not declared"}",
                 $"Build: {package.BuildDate?.ToLocalTime().ToString("g", CultureInfo.CurrentCulture) ?? "Not declared"}",
    ...
    public string SelectedValidationDetails => SelectedPackage is null
        ? _localization.Get("MapNoPackageSelectedPeriod")
    ...
    private async Task ImportDeploymentAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            RefreshPresentation(result.Packages.Count > 0 ? result.Packages[0].Key : null);
    ...
    private void OnLocalizationChanged(object? sender, System.ComponentModel.PropertyChangedEventArgs e)
    {
        ...
        OnPropertyChanged(nameof(SelectedPackageSubtitle));
        OnPropertyChanged(nameof(SelectedValidationDetails));
        if (string.Equals(StatusMessage, "Ready.", StringComparison.Ordinal))
        {
            StatusMessage = _localization.Get("MapReady");
        }
    }
    ...
    private static readonly char[] DocumentPathSeparators = [';', '\r', '\n'];

    private static string[] ParseDocumentPaths(string value)
        => value.Split(DocumentPathSeparators, StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
