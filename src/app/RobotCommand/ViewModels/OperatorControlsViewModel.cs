...
public sealed class OperatorControlsViewModel : ObservableObject
{
    ...
    public MapGoToTargetVisual? ActiveGoToTarget => ActiveGoToTargets.FirstOrDefault();
    ...
    public bool HasQueuedCommandsForSelection => GetSelectedQueuedPlans().Count > 0;

    public bool HasQueuedCommand => GetSelectedQueuedPlans().Count > 0;

    public bool HasExecutingCommand => GetSelectedExecutingPlans().Count > 0;
    ...
    public string SelectedQueuedAction
    {
        get
        {
            ...
            return plans.Count == 1
                    ...
                    : $"{plans.Count} queued actions · {QueueAvailability(plans)}";
        }
    }
    ...
    public string SelectedQueuedAvailability
    {
        get
        {
            ...
            return unavailable > 0
                ? $"Unavailable for {unavailable} of {plans.Count} units: {plans.First(plan => plan.Availability is OperatorWorkflowAvailability.Blocked or OperatorWorkflowAvailability.Unavailable).Findings.FirstOrDefault(item => item.Severity == OperatorWorkflowSeverity.Blocking)?.Message ?? "readiness or policy check failed"}"
                : warnings > 0
                    ? $"Available with warnings for {warnings} of {plans.Count} units"
                    : $"Available for {plans.Count} units";
        }
    }
    ...
    {
        var vehicleId = TargetUnitIds.FirstOrDefault();
    ...
    public bool PendingExecutionReady => _pendingPlans.Count > 0 &&
        _pendingPlans.All(plan => (plan.Availability is OperatorWorkflowAvailability.Ready or OperatorWorkflowAvailability.Warning) &&
                                   plan.ExpiresAt > DateTimeOffset.UtcNow);

    public bool PendingExecutionBlocked => HasPendingPlan && !PendingExecutionReady;
    ...
    private async Task PrepareMapCommandAsync(OperatorCommandKind command, object? parameter)
    {
        ...
        try
        {
            ...
            await QueueAsync(command, parameters, CancellationToken.None, null);
        }
        finally
        {
        ...
        string? pendingLabel)
    {
        ...
        try
        {
            ...
            PendingPlan = _pendingPlans.FirstOrDefault();
            ConfirmationText = "";
            ReplaceFindings(_pendingPlans.SelectMany(plan => plan.Findings));
            var blocked = _pendingPlans.Any(plan => plan.Availability is OperatorWorkflowAvailability.Blocked or OperatorWorkflowAvailability.Unavailable);
            ...
            StatusMessage = blocked
                ...
                : _pendingPlans.Count > 1
                    ...
                    : _pendingPlans.FirstOrDefault()?.Availability switch
                    {
    ...
    private async Task ExecutePendingAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            StatusMessage = results.Count > 1
                ...
                : results.FirstOrDefault()?.Message ?? "No queued commands were executed.";
            PendingPlan = null;
            _pendingPlans = [];
    ...
    private async Task ExecuteQueuedForSelectionAsync()
    {
        ...
        StatusMessage = queued.Count > 1 ? $"Executed {queued.Count} queued unit commands." : "Queued command executed.";
    ...
    private bool CanExecutePending()
        => GetSelectedQueuedPlans().Count > 0 &&
           GetSelectedQueuedPlans().All(plan => (plan.Availability is OperatorWorkflowAvailability.Ready or OperatorWorkflowAvailability.Warning) && plan.ExpiresAt > DateTimeOffset.UtcNow);

    private bool CanExecuteQueuedForSelection()
        => GetSelectedQueuedPlans().Count > 0 &&
           GetSelectedQueuedPlans().All(plan => (plan.Availability is OperatorWorkflowAvailability.Ready or OperatorWorkflowAvailability.Warning) && plan.ExpiresAt > DateTimeOffset.UtcNow);

    private IReadOnlyList<OperatorCommandQueueSnapshot> GetSelectedQueuedPlans()
    ...
    private IReadOnlyList<OperatorCommandQueueSnapshot> GetSelectedExecutingPlans()
        => _workflow.ActiveCommands.Where(plan => SelectedCommandVehicleIds.Contains(plan.UnitId, StringComparer.Ordinal)).ToArray();

    private bool CanCancelExecuting() => GetSelectedExecutingPlans().Count > 0;
    ...
    private void ApplySelection()
    {
        ...
        var newVehicleId = TargetUnitIds.FirstOrDefault();
        ...
        {
            _selectedVehicleId = targetScope.UnitIds.FirstOrDefault();
    ...
    private static OperatorWorkflowParameters ToWorkflow(OperatorCommandParameters parameters) => new(
        ...
        parameters.HeadingDegrees, parameters.RelativeYawDegrees);
