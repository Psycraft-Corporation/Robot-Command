...
public sealed class CameraPanelViewModel : ObservableObject
{
    ...
    private string _cameraStatus = "No camera source selected";
        ...
        if (_activeStream is not null)
        {
            ...
            if (_activeStream is not null && IsTerminal(_activeStream))
            {
                _ = ScheduleFallbackAsync(
                    ...
                    $"Logos reported terminal stream state '{_activeStream.State}'.");
            }
        }
    ...
    private void RefreshPresentation()
    {
        ...
        if (camera is null)
        {
            CameraStatus = "No camera source available for the selected vehicle connection.";
            OverlayScene = VideoOverlayScene.Empty;
            TrackSummary = "No perception tracks";
        }
        else
        {
            ...
            TrackSummary = relevantTracks.Length == 0
                ? "No perception tracks"
                : $"{relevantTracks.Length} track(s) overlaid";
        }

        if (_activeStream is null)
        {
            StreamStatus = "No Logos stream";
            StreamEndpoint = "-";
        }
        else
        {
    ...
    private bool IsGhostCamera(CameraSourceRecord camera)
    ...
    {
        var status = _playback.Status;
        PlaybackSummary = status.Summary;
        PlaybackDetail = status.Detail;
    ...
    private void ApplyNativeStatus()
    {
        ...
        NativeVideoStatus = status.Summary;
        NativeVideoDetail = status.Detail;
    ...
    private void ApplyLocalVideoStatus()
    {
        ...
        LocalRecordingStatus = status.Summary;
        LocalRecordingDetail = status.Detail;
        ...
        TimelineRangeText = timeline.RangeStart is null || timeline.RangeEnd is null
            ? "No console or vehicle recordings"
