...
public sealed class OperationalMapViewModel : ObservableObject, IMapNavigationController, IDisposable
{
    ...
    private readonly AsyncRelayCommand _toggleWeatherCommand;
    ...
    private string _savedViewStatus = string.Empty;
    private MapStyleOption? _selectedStyle;
    private ThreeDSceneSnapshot? _worldSceneSnapshot;
    private WeatherRadarSnapshot _weatherRadarSnapshot = WeatherRadarSnapshot.Empty;
    private bool _weatherVisible;
    private double _weatherOpacity = 0.55;
        ...
        ILocalizationService? localization = null)
    {
        ...
        if (_weatherRadar is not null) _weatherRadar.Changed += OnWeatherRadarChanged;
        ...
        _toggleWeatherCommand = new AsyncRelayCommand(
            ToggleWeatherAsync,
            () => GlobalMapControlsAvailable && _weatherRadar is not null);
    ...
    public WeatherRadarSnapshot WeatherRadar => _weatherRadarSnapshot;
    ...
    public bool WeatherVisible
    {
        ...
        private set
        {
            if (SetProperty(ref _weatherVisible, value))
            {
                OnPropertyChanged(nameof(WeatherButtonLabel));
                OnPropertyChanged(nameof(WeatherStatus));
            }
        }
    }

    public double WeatherOpacity
    {
        get => _weatherOpacity;
        set
        {
            var next = Math.Clamp(value, 0.2, 0.85);
            if (SetProperty(ref _weatherOpacity, next))
            {
    ...
    public string WeatherStatus => WeatherVisible ? _weatherRadarSnapshot.Status : "Weather off";

    public string WeatherButtonLabel => WeatherVisible
        ? _localization.Get("MapHideWeather")
        : _localization.Get("MapWeatherRadar");
    ...
    public ICommand ToggleWeatherCommand => _toggleWeatherCommand;
    ...
    public MapStyleOption? SelectedStyle
    {
        ...
        set
        {
            ...
            // MapStyleOption is a record, so value equality treats a freshly
            // rebuilt style item as unchanged. The ComboBox needs the new
            // collection item reference after Styles.Clear()/Add(), otherwise
            // its selected value remains blank even though the style ID is
            // still current.
    ...
    public string OperatorNavigationLabel => _localization.Get(
        CanJumpToOperator ? "MapJumpToOperator" : "MapOperatorUnavailable");

    public string GeometryButtonLabel => _localization.Get(
        GeometryVisible ? "MapHideGeometries" : "MapShowGeometries");

    public string PolicyButtonLabel => _localization.Get(
        PolicyVisible ? "MapHidePolicy" : "MapShowPolicy");

    public string TrailsButtonLabel => _localization.Get(
        TrailsVisible ? "MapHideTrails" : "MapShowTrails");

    public string GoToIndicatorsButtonLabel => _localization.Get(
        GoToIndicatorsVisible ? "MapHideGoTo" : "MapShowGoTo");

    public string VehicleLabelsButtonLabel => _localization.Get(
        VehicleLabelsVisible ? "MapHideLabels" : "MapShowLabels");

    public string OrientationButtonLabel => _localization.Get(
        OrientationMode == MapOrientationMode.NorthUp ? "MapNorthUp" : "MapCourseUp");
    ...
    public static string GeometryAltitudeUnitLabel => "m AGL";
        ...
        try
        {
            ...
            if (_geometryWorkflow is not null && _geometryCodec is not null)
            {
                await _geometryWorkflow.SaveAsync(
                    new GeometrySaveWorkflowRequest(GeometryDocumentCodec.Serialize(completed)),
                    cancellationToken);
            }
            else
            {
    ...
    private async Task ApplySavedViewAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            WeatherOpacity = view.WeatherOpacity;
            await SetWeatherVisibleAsync(view.WeatherVisible, cancellationToken);
    ...
    private void RefreshStyles()
    {
        ...
        var availableStyles = _engine.AvailableStyles.ToList();
        availableStyles.Add(new MapStyleOption("3d", "3D", MapStyleKind.Operational, [], string.Empty, false)
        {
            IsThreeD = true
        });
        ...
        try
        {
            // A normal basemap switch raises the engine Changed event, but it
            // does not change the available style list. Keep the collection
            // intact in that case so Avalonia does not clear the ComboBox's
            // selected value while the new basemap is being applied.
            if (!StyleListsMatch(availableStyles))
            {
                Styles.Clear();
    ...
    private bool StyleListsMatch(List<MapStyleOption> expected)
        => Styles.Count == expected.Count &&
           Styles.Zip(expected).All(pair =>
               string.Equals(pair.First.StyleId, pair.Second.StyleId, StringComparison.OrdinalIgnoreCase) &&
               string.Equals(pair.First.DisplayName, pair.Second.DisplayName, StringComparison.Ordinal) &&
               pair.First.Kind == pair.Second.Kind &&
               string.Equals(pair.First.Attribution, pair.Second.Attribution, StringComparison.Ordinal) &&
               pair.First.Default == pair.Second.Default &&
               pair.First.IsThreeD == pair.Second.IsThreeD &&
               pair.First.Layers.SequenceEqual(pair.Second.Layers));
    ...
    private void Rebuild()
    {
        ...
        Presentation = prepared with
        {
            ...
            Motion = motion,
            WeatherRadar = _weatherRadarSnapshot,
            WeatherVisible = WeatherVisible,
            WeatherOpacity = WeatherOpacity
        };
        ...
        OnPropertyChanged(nameof(IsThreeD));
        OnPropertyChanged(nameof(WorldCameraModeLabel));
        _toggleWeatherCommand.RaiseCanExecuteChanged();
        _saveViewCommand.RaiseCanExecuteChanged();
    ...
    {
        _mapVisible = visible;
        _weatherRadar?.SetViewActive(visible && !IsThreeD);
    }

    private Task ToggleWeatherAsync(CancellationToken cancellationToken)
        => SetWeatherVisibleAsync(!WeatherVisible, cancellationToken);

    private async Task SetWeatherVisibleAsync(bool visible, CancellationToken cancellationToken)
    {
        if (_weatherRadar is null)
        {
        ...
        try
        {
            await _weatherRadar.SetEnabledAsync(visible, cancellationToken);
            WeatherVisible = visible;
            _weatherRadarSnapshot = _weatherRadar.Current;
            UpdateWeatherPresentation();
        }
        catch (OperationCanceledException)
        {
        ...
        {
            WeatherVisible = false;
            _weatherRadarSnapshot = _weatherRadar.Current with
            {
                Status = $"Weather radar unavailable: {ex.Message}"
            };
            UpdateWeatherPresentation();
        }
    }

    private void OnWeatherRadarChanged(object? sender, EventArgs e)
    {
        if (_weatherRadar is null)
        {
            return;
        ...
        {
            _weatherRadarSnapshot = _weatherRadar.Current;
            OnPropertyChanged(nameof(WeatherRadar));
            OnPropertyChanged(nameof(WeatherStatus));
            UpdateWeatherPresentation();
        });
    }
    ...
    {
        var selectedIds = ActiveUnitSelectionIds();
        var selectedId = selectedIds.Count > 0 ? selectedIds[0] : null;
    ...
    private HashSet<string> HighlightedGeometryIds()
    {
