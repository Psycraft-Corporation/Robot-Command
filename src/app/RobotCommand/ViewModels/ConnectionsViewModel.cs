...
{
    private static readonly Action<ILogger, Exception?> SerialDiscoveryFailedLog =
        LoggerMessage.Define(LogLevel.Warning, new EventId(1001, nameof(SerialDiscoveryFailedLog)), "Serial-device discovery failed");
    private static readonly Action<ILogger, Exception?> RadioPairingFailedLog =
        LoggerMessage.Define(LogLevel.Warning, new EventId(1002, nameof(RadioPairingFailedLog)), "SiK radio pairing failed");
    private static readonly Action<ILogger, string, Exception?> OperationFailedLog =
        LoggerMessage.Define<string>(LogLevel.Warning, new EventId(1009, nameof(OperationFailedLog)), "{Operation} failed");
    ...
    public SerialDeviceDescriptor? SelectedSerialDevice
    {
        ...
        set
        {
            ...
            if (value is not null && (IsSerialMavlink || IsLinkdConnection))
            {
                ApplySelectedSerialDevice(value);
            }
            RestoreRadioProbeForSelectedRadio();
            RaiseCommandStates();
        }
    ...
    public string LastConnectedAt => SelectedConnection?.LastConnectedAt?.ToLocalTime().ToString("G", CultureInfo.CurrentCulture) ?? "Never";
    public string LastSeen => SelectedConnection?.LastSeen?.ToLocalTime().ToString("G", CultureInfo.CurrentCulture) ?? "Never";
    public string LastAttempt => SelectedConnection?.LastAttempt?.ToLocalTime().ToString("G", CultureInfo.CurrentCulture) ?? "Never";
    ...
    private ConnectionDefinition BuildDefinition(string id)
    {
        ...
        if (Mode == ConnectionMode.Mavlink)
        {
            if (!TryParseAliases(_mavlinkSystemAliases, out var aliases))
            {
    ...
    private async Task DetectSerialDevicesAsync(CancellationToken cancellationToken)
    {
        ...
        {
            SerialDiscoveryFailedLog(_logger, ex);
        }
    }

    private void ApplySelectedSerialDevice(SerialDeviceDescriptor device)
    {
    ...
    private async Task PairRadiosAsync(CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            RadioPairingFailedLog(_logger, ex);
        }
        finally
        {
            RaiseCommandStates();
    ...
    private void ReplaceTiles()
    {
        ...
        Tiles.Add(new ConnectionMetric(localization.Get("ConnectionState"), state?.ToString() ?? "-", connection?.LastError ?? "", StateColor(state)));
        Tiles.Add(new ConnectionMetric(localization.Get("ConnectionProtocol"), Protocol, connection?.Target ?? ""));
        Tiles.Add(new ConnectionMetric(localization.Get("ConnectionConnected"), ConnectedAt));
        Tiles.Add(new ConnectionMetric(localization.Get("ConnectionLinks"), Links.Count.ToString(CultureInfo.CurrentCulture)));
        Tiles.Add(new ConnectionMetric(localization.Get("ConnectionEvents"), Events.Count.ToString(CultureInfo.CurrentCulture)));
    }

    private static IReadOnlyList<ConnectionMetric> EmptyTiles()
    {
        var localization = LocalizationService.Current;
        return [
            new(localization.Get("ConnectionState"), "-"),
            new(localization.Get("ConnectionProtocol"), "-"),
            new(localization.Get("ConnectionConnected"), "-"),
            new(localization.Get("ConnectionLinks"), "-"),
            new(localization.Get("ConnectionEvents"), "-")];
    }

    public string ProtocolSummary => $"{LocalizationService.Current.Get("ConnectionProtocol")}: {Protocol}";
    public string BaudRateSummary => $"{LocalizationService.Current.Get("ConnectionBaudRate")}: {BaudRate}";
    public string ConnectedSinceSummary => $"{LocalizationService.Current.Get("ConnectionConnectedSince")}: {ConnectedAt}";
    public string LastConnectedSummary => $"{LocalizationService.Current.Get("ConnectionLastConnected")}: {LastConnectedAt}";
    public string LastSeenSummary => $"{LocalizationService.Current.Get("ConnectionLastSeen")}: {LastSeen}";
    public string LastAttemptSummary => $"{LocalizationService.Current.Get("ConnectionLastAttempt")}: {LastAttempt}";

    private void OnLocalizationChanged(object? sender, System.ComponentModel.PropertyChangedEventArgs e)
    {
        ReplaceTiles();
        OnPropertyChanged(nameof(ProtocolSummary));
        OnPropertyChanged(nameof(BaudRateSummary));
        OnPropertyChanged(nameof(ConnectedSinceSummary));
        OnPropertyChanged(nameof(LastConnectedSummary));
        OnPropertyChanged(nameof(LastSeenSummary));
        OnPropertyChanged(nameof(LastAttemptSummary));
    }
    ...
    {
        foreach (var command in new[] { AddCommand, SaveCommand, DeleteCommand, ConnectCommand, DisconnectCommand, RefreshCommand, ProbeRadioCommand, PairRadiosCommand, ApplyLocalRadioCommand, ApplyPairedRadioCommand, ConfirmRadioApplyCommand })
            if (command is AsyncRelayCommand async) async.RaiseCanExecuteChanged();
        foreach (var command in new[] { ToggleConnectionCommand, ReviewLocalRadioCommand, ReviewPairedRadioCommand, CancelRadioApplyCommand })
            if (command is RelayCommand relay) relay.RaiseCanExecuteChanged();
    }
    private void Fail(string operation, Exception ex) { ActionStatus = $"{operation} failed: {ex.Message}"; OperationFailedLog(_logger, operation, ex); }
