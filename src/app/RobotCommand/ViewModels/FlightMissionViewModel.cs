...
public sealed class FlightMissionViewModel : ObservableObject, IDisposable
{
    ...
    private string _newName = "New mission";
        ...
        IReviewedOperationWorkflow reviewed, IOperatorLocationService operatorLocation, IUiDispatcher dispatcher)
    {
        ...
        ValidateCommand = new AsyncRelayCommand(ValidateAsync, HasMission);
        AddTakeoffCommand = new AsyncRelayCommand(token => AddAsync(() => _workflow.AddTakeoffAsync(Require().Id, token)), HasMission);
        AddRtlCommand = new AsyncRelayCommand(token => AddAsync(() => _workflow.AddReturnToLaunchAsync(Require().Id, token)), HasMission);
        AddLandCommand = new AsyncRelayCommand(token => AddAsync(() => _workflow.AddLandAsync(Require().Id, token)), HasMission);
        ...
        AddCameraIntentCommand = new AsyncRelayCommand(token => AddAsync(() => _workflow.AddCameraIntentAsync(
            ...
            token)), HasMission);
        SetAltitudeCommand = new AsyncRelayCommand(SetAltitudeAsync, HasMission);
        SetSpeedCommand = new AsyncRelayCommand(SetSpeedAsync, () => HasMission() && CruiseSpeed > 0);
        ...
        SetFenceCommand = new AsyncRelayCommand(SetFenceAsync, HasMission);
        ...
        SetEndActionCommand = new AsyncRelayCommand(SetEndActionAsync, HasMission);
        ...
        _workflow.Changed += OnChanged; _geometry.Changed += OnChanged; _units.Changed += OnChanged; _fences.Changed += OnChanged; _reviewed.Changed += OnChanged; _operatorLocation.Changed += OnChanged; Refresh();
    }
    ...
    public FlightMissionSnapshot? SelectedMission
    {
        ...
        set
        {
            ...
            NewName = value?.Name ?? "New mission";
    ...
    public string PreviewSummary => Preview?.Summary ?? "Select a mission to preview its route.";
    private bool HasMission() => SelectedMission is not null; private bool HasTarget() => SelectedTarget is not null; private bool CanOperate() => HasMission() && HasTarget();
    ...
    private async Task RemoveStepAsync(CancellationToken token) { var step = Require().Steps.LastOrDefault(); if (step is not null) { SelectMissionSnapshot(await _workflow.RemoveStepAsync(Require().Id, step.Id, token)); Status = "Last mission step removed."; } }
    ...
    private async Task<ReviewedOperationSnapshot?> CreatePlanAsync(string operation, CancellationToken token)
    {
        try
        {
            ...
            var connection = target.CommandAuthorityVehicleId ?? target.ConnectionIds.FirstOrDefault() ?? throw new InvalidOperationException("The selected unit has no mission connection.");
    ...
    private static string HumanReadableBlock(ReviewedOperationSnapshot operation)
    {
        var finding = operation.Findings.FirstOrDefault(item => item.Severity == WorkflowFindingSeverity.Blocking)
            ?? operation.Findings.FirstOrDefault();
    ...
    private void RefreshCore()
    {
        ...
        OnPropertyChanged(nameof(SelectedMission)); OnPropertyChanged(nameof(SelectedGeometry)); OnPropertyChanged(nameof(SelectedTarget)); OnPropertyChanged(nameof(PreviewUnit)); OnPropertyChanged(nameof(PreviewOperatorLocation)); OnPropertyChanged(nameof(SelectedFence)); OnPropertyChanged(nameof(SelectedStep)); OnPropertyChanged(nameof(PendingOperation)); OnPropertyChanged(nameof(HasPendingOperation)); OnPropertyChanged(nameof(PendingOperationOutcome)); OnPropertyChanged(nameof(Execution)); OnPropertyChanged(nameof(HasExecution)); OnPropertyChanged(nameof(IsAwaitingPostLandingDecision)); OnPropertyChanged(nameof(ExecutionStatusText)); OnPropertyChanged(nameof(CanStartMission)); OnPropertyChanged(nameof(StartMissionUnavailableReason)); RaiseCommands();
        RequestPreview();
    }
    ...
    public void Dispose() { _previewCancellation?.Cancel(); _previewCancellation?.Dispose(); _stepOptionsCancellation?.Cancel(); _stepOptionsCancellation?.Dispose(); _workflow.Changed -= OnChanged; _geometry.Changed -= OnChanged; _units.Changed -= OnChanged; _fences.Changed -= OnChanged; _reviewed.Changed -= OnChanged; _operatorLocation.Changed -= OnChanged; }
}
