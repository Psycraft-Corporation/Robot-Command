...
public sealed class UnitsPanelViewModel : ObservableObject
{
    ...
    private bool _isEditingUnitConnections;
    private string? _editingUnitId;
    private string _unitDisplayName = string.Empty;
    private UnitConnectionOption? _commandAuthority;
    private UnitConnectionOption? _telemetryAuthority;
    private UnitConnectionOption? _diagnosticsAuthority;
    private string _unitConnectionStatus = string.Empty;
        ...
        IFormationAuthoringWorkflow? formationAuthoring = null)
    {
        ...
        AvailableUnitConnections = new ObservableCollection<UnitConnectionOption>();
        IncludedUnitConnections = new ObservableCollection<UnitConnectionOption>();
        ...
        SaveUnitConnectionsCommand = new AsyncRelayCommand(SaveUnitConnectionsAsync, CanSaveUnitConnections);
        CancelUnitConnectionsCommand = new RelayCommand(_ => IsEditingUnitConnections = false);
        RemoveManualUnitDefinitionCommand = new AsyncRelayCommand(RemoveManualUnitDefinitionAsync, () => IsSelectedManuallyDefined);
    ...
    public string TeamFormationMemberStatus => !IsTeamFormationLocked ? string.Empty : string.Join(" · ", Formation.Members.Select(member => $"{member.Name}: {member.Backend ?? "formation"} {member.ControlState}"));
    ...
    public ObservableCollection<UnitConnectionOption> AvailableUnitConnections { get; }
    public ObservableCollection<UnitConnectionOption> IncludedUnitConnections { get; }
    ...
    public bool IsSelectedManuallyDefined
        => SelectedUnit?.VehicleId is { } id && _reconciliation?.FindBySource(id) is not null;
    ...
    public bool IsEditingUnitConnections
    {
        get => _isEditingUnitConnections;
        private set => SetProperty(ref _isEditingUnitConnections, value);
    }
    public string UnitDisplayName
    {
        ...
        set
        {
            if (!SetProperty(ref _unitDisplayName, value)) return;
            (SaveUnitConnectionsCommand as AsyncRelayCommand)?.RaiseCanExecuteChanged();
        }
    }
    public UnitConnectionOption? CommandAuthority
    {
        get => _commandAuthority;
        set
        {
            if (!SetProperty(ref _commandAuthority, value)) return;
            (SaveUnitConnectionsCommand as AsyncRelayCommand)?.RaiseCanExecuteChanged();
        }
    }
    public UnitConnectionOption? TelemetryAuthority
    {
        get => _telemetryAuthority;
        set
        {
            if (!SetProperty(ref _telemetryAuthority, value)) return;
            (SaveUnitConnectionsCommand as AsyncRelayCommand)?.RaiseCanExecuteChanged();
        }
    }
    public UnitConnectionOption? DiagnosticsAuthority
    {
        get => _diagnosticsAuthority;
        set
        {
            if (!SetProperty(ref _diagnosticsAuthority, value)) return;
            (SaveUnitConnectionsCommand as AsyncRelayCommand)?.RaiseCanExecuteChanged();
        }
    }
    public string UnitConnectionStatus
    {
        get => _unitConnectionStatus;
        private set => SetProperty(ref _unitConnectionStatus, value);
    }
    ...
    public string SelectedTitle => SelectedUnit?.VehicleName ?? SelectedUnit?.Name ?? "No unit selected";
    public string SelectionSummary => $"{SelectedUnitCount} units selected";
    public string SelectedSubtitle => SelectedUnit is null
        ? "Connect a Logos runtime to discover units."
        : string.IsNullOrWhiteSpace(SelectedUnit.Role)
            ? SelectedUnit.PlatformKind
            : SelectedUnit.Role;
    public string SelectedState => SelectedObservation?.State.ToString() ?? SelectedUnit?.State.ToString() ?? "-";
    ...
    public string SelectedOverallStatus => SelectedObservation?.Diagnostics?.OverallStatus ?? SelectedDiagnostics?.OverallStatus.ToString() ?? "Unknown";
    public string SelectedBlockerSummary => SelectedObservation?.Diagnostics is { } observedDiagnostics
        ? observedDiagnostics.Blockers.Count == 0 ? "No current blockers" : $"{observedDiagnostics.Blockers.Count} blocker(s)"
        : SelectedDiagnostics is { } diagnostics
            ? diagnostics.Blockers.Count == 0 ? "No current blockers" : $"{diagnostics.Blockers.Count} blocker(s)"
    ...
    public string SelectedMissionExecutionProgress => SelectedMissionExecution is { CurrentItemIndex: { } index, ItemCount: > 0 } execution
        ? $"Route item {Math.Min(index + 1, execution.ItemCount)} of {execution.ItemCount}"
        : string.Empty;
    ...
    public string SelectedBatteryTooltip
    {
        get
        {
            ...
            if (telemetry?.BatteryRemainingPercent is not double percent || !double.IsFinite(percent))
                return "Battery percentage was not reported by this vehicle.";
            ...
            var freshness = telemetry.BatteryObservedAt is { } observedAt
                ...
                : string.Empty;
            return $"Battery {percent:0}%{voltage}.{freshness} Source: authoritative vehicle diagnostics.";
        }
    }

    public string SelectedSignalTooltip
        => SelectedConnectionChips.Count == 0
            ? "No connection signal metrics were reported."
    ...
    public string SelectedConnections => SelectedUnit?.ConnectionIds?.Count.ToString() ?? "0";
    ...
    public ICommand SaveUnitConnectionsCommand { get; }
    public ICommand CancelUnitConnectionsCommand { get; }
    public ICommand RemoveManualUnitDefinitionCommand { get; }
    ...
    public async Task CreateTeamFromSelectionAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            await _teams.CreateAsync(ids, cancellationToken: cancellationToken);
    ...
    {
        if (!CanManageUnitConnections || SelectedUnit?.VehicleId is not { } selectedVehicleId || _vehicleStore is null)
            return;

        var definition = _reconciliation?.FindBySource(selectedVehicleId);
        var included = definition?.Connections.ToHashSet()
                       ?? BindingsForVehicle(selectedVehicleId).ToHashSet();

        AvailableUnitConnections.Clear();
        foreach (var vehicle in _vehicleStore.Items.OrderBy(item => item.Name, StringComparer.OrdinalIgnoreCase))
        {
            foreach (var connectionId in vehicle.ConnectionIds.Distinct(StringComparer.Ordinal))
            {
                var connection = _connectionStore.Items.FirstOrDefault(item => item.Id == connectionId);
                if (connection is null || connection.Mode == ConnectionMode.Ghost) continue;
                var owner = _reconciliation?.FindByBinding(connection.Id, vehicle.Id);
                if (owner is not null && owner.Id != definition?.Id) continue;
                var option = new UnitConnectionOption(
                    connection.Id,
                    vehicle.Id,
                    connection.Name,
                    connection.Mode == ConnectionMode.Mavlink ? "MAVLink" : "Logos",
                    connection.Target,
                    vehicle.Name,
                    included.Contains(new UnitConnectionBinding(connection.Id, vehicle.Id)));
                option.InclusionChanged += OnUnitConnectionInclusionChanged;
                AvailableUnitConnections.Add(option);
            }
        }

        _editingUnitId = definition?.Id;
        UnitDisplayName = definition?.DisplayName ?? SelectedTitle;
        RefreshIncludedUnitConnections(
            definition?.CommandAuthorityConnectionId,
            definition?.TelemetryAuthorityConnectionId,
            definition?.DiagnosticsAuthorityConnectionId);
        UnitConnectionStatus =
            "Associate connections with this physical unit, then choose the authoritative connection for each responsibility.";
        IsEditingUnitConnections = true;
        (SaveUnitConnectionsCommand as AsyncRelayCommand)?.RaiseCanExecuteChanged();
    }

    private IReadOnlyList<UnitConnectionBinding> BindingsForVehicle(string vehicleId)
        => _vehicleStore?.TryGet(vehicleId, out var vehicle) == true && vehicle is not null
            ? vehicle.ConnectionIds.Select(connectionId => new UnitConnectionBinding(connectionId, vehicle.Id)).ToArray()
            : [];

    private void OnUnitConnectionInclusionChanged(object? sender, EventArgs e)
        => RefreshIncludedUnitConnections(
            CommandAuthority?.ConnectionId,
            TelemetryAuthority?.ConnectionId,
            DiagnosticsAuthority?.ConnectionId);

    private void RefreshIncludedUnitConnections(
        string? commandConnectionId,
        string? telemetryConnectionId,
        string? diagnosticsConnectionId)
    {
        IncludedUnitConnections.Clear();
        foreach (var option in AvailableUnitConnections.Where(item => item.IsIncluded))
            IncludedUnitConnections.Add(option);

        CommandAuthority = FindIncludedConnection(commandConnectionId)
                           ?? IncludedUnitConnections.FirstOrDefault(item => item.Backend == "Logos")
                           ?? IncludedUnitConnections.FirstOrDefault();
        TelemetryAuthority = FindIncludedConnection(telemetryConnectionId)
                             ?? IncludedUnitConnections.FirstOrDefault(item => item.Backend == "MAVLink")
                             ?? CommandAuthority;
        DiagnosticsAuthority = FindIncludedConnection(diagnosticsConnectionId)
                               ?? IncludedUnitConnections.FirstOrDefault(item => item.Backend == "MAVLink")
                               ?? TelemetryAuthority;
        (SaveUnitConnectionsCommand as AsyncRelayCommand)?.RaiseCanExecuteChanged();
    }

    private UnitConnectionOption? FindIncludedConnection(string? connectionId)
        => string.IsNullOrWhiteSpace(connectionId)
            ? null
            : IncludedUnitConnections.FirstOrDefault(item => item.ConnectionId == connectionId);

    private bool CanSaveUnitConnections()
        => _reconciliation is not null &&
           IncludedUnitConnections.Select(item => item.ConnectionId).Distinct(StringComparer.Ordinal).Count() >= 2 &&
           !string.IsNullOrWhiteSpace(UnitDisplayName) &&
           CommandAuthority is not null && TelemetryAuthority is not null && DiagnosticsAuthority is not null;

    private async Task SaveUnitConnectionsAsync(CancellationToken cancellationToken)
    {
        if (!CanSaveUnitConnections() || _reconciliation is null) return;
        try
        {
            var definition = await _reconciliation.SaveAsync(
                _editingUnitId,
                UnitDisplayName,
                IncludedUnitConnections.Select(item => new UnitConnectionBinding(item.ConnectionId, item.VehicleId)).ToArray(),
                CommandAuthority!.ConnectionId,
                TelemetryAuthority!.ConnectionId,
                DiagnosticsAuthority!.ConnectionId,
                cancellationToken);
            IsEditingUnitConnections = false;
            SelectAuthorityVehicle(definition);
        }
        catch (Exception ex) when (ex is InvalidOperationException or IOException or UnauthorizedAccessException)
        {
            UnitConnectionStatus = ex.Message;
        }
    }

    private async Task RemoveManualUnitDefinitionAsync(CancellationToken cancellationToken)
    {
        if (SelectedUnit?.VehicleId is not { } vehicleId ||
            _reconciliation?.FindBySource(vehicleId) is not { } definition)
            return;
        await _reconciliation.RemoveAsync(definition.Id, cancellationToken);
        IsEditingUnitConnections = false;
        SelectAuthorityVehicle(definition);
    }

    private void SelectAuthorityVehicle(ManualUnitDefinition definition)
    {
        var commandVehicleId = definition.Connections.First(item =>
            item.ConnectionId == definition.CommandAuthorityConnectionId).VehicleId;
        if (_vehicleStore?.TryGet(commandVehicleId, out var vehicle) == true && vehicle is not null)
    ...
    private async Task CreateGhostAsync(CancellationToken cancellationToken)
    {
        if (_ghostWorkflow is not null)
        {
            ...
            if (created.Count > 0)
                SelectedUnit = _runtimeStore.Items.FirstOrDefault(item => item.VehicleId == created[^1].UnitId);
            return;
        }
        if (_ghosts is null) return;
        var viewport = _viewportState?.Current;
        VehicleRecord? lastVehicle = null;
        ...
        for (var index = 0; index < GhostCount; index++)
            lastVehicle = await _ghosts.CreateAsync(fallbackProfileId, viewport, cancellationToken);

        IsCreatingGhost = false;
        if (lastVehicle is not null)
            SelectedUnit = _runtimeStore.Items.FirstOrDefault(item => item.VehicleId == lastVehicle.Id);
    }
    ...
    {
        if (_formation is null || SelectedTeam?.TeamId is not { } teamId) return;
        PendingFormationOperation = await _formation.PlanHoldAsync(teamId, cancellationToken);
        SetTeamStatus(PendingFormationOperation.Summary);
        OnFormationChanged();
    }
    ...
    private void OnFormationChanged()
    {
        ...
        OnPropertyChanged(nameof(TeamFormationMemberStatus));
    ...
    {
        var selectedVehicleId = _selection.SelectedUnitIds.FirstOrDefault();
    ...
    private void RefreshDetails()
    {
        ...
        OnPropertyChanged(nameof(HasMultipleSelectedUnits));
        OnPropertyChanged(nameof(IsSelectedManuallyDefined));
        ...
        (RemoveManualUnitDefinitionCommand as AsyncRelayCommand)?.RaiseCanExecuteChanged();
    }
...
public sealed class UnitListItemViewModel : ObservableObject
{
    ...
    public string ConnectionLabel => $"{ConnectionCount} associated connection(s)";
    ...
    public string TeamTooltip => IsInTeam
        ? string.Format(LocalizationService.Current.Get("TeamTooltip"), _teamName, _teamOrder + 1, _teamMemberCount)
        : string.Empty;
    ...
    {
        if (isGhost) return string.Empty;
    ...
    private static IReadOnlyList<UnitStatusIndicatorViewModel> BuildStatusIndicators(UnitObservationSnapshot? observation)
    {
        ...
        var batteryTooltip = battery == "—" ? "Battery percentage was not reported by this vehicle." : $"Battery {battery}.";
        ...
        var signal = links.Count == 0
            ...
            : links.Count == 1 && links[0].Quality is double quality
                ...
                : links.Count == 1 ? "—" : $"{links.Count} links";
        ...
        return
        [
            new UnitStatusIndicatorViewModel(signalSymbol, signal, signalBrush, links.Count == 0 ? "Simulator link quality: 100%." : "Connection signal quality."),
...
public sealed class UnitConnectionOption : ObservableObject
{
        ...
        bool isIncluded)
    {
        ...
        Backend = backend;
        Target = target;
        ObservedUnitName = observedUnitName;
    ...
    public string ObservedUnitName { get; }
    public string Detail => Target;
    public string Label => $"{ConnectionName} · {Backend} · {ObservedUnitName} · {Target}";
