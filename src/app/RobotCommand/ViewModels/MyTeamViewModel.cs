...
public sealed class MyTeamViewModel : ObservableObject
{
    ...
    private string _remoteDisplayName = Environment.MachineName;
    ...
    public string RemotePassphrase { get => _remotePassphrase; set => SetProperty(ref _remotePassphrase, value); }
    public string ServerPassphrase { get => _serverPassphrase; set => SetProperty(ref _serverPassphrase, value); }
    ...
    public string PairingExpires => _pairing.CurrentInvitation is { } invitation
        ...
        : "Create a QR pairing invitation to connect another Robot Command or compatible observer.";
    public bool HasPairingInvitation => _pairing.CurrentInvitation is not null;

    public string Title => L("MyTeam");
    public string Description => L("MyTeamDescription");
    ...
    public string DiscoveryStatus => _discovery.Status;
    ...
    private async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        {
            RemoteStatus = "Enter a server passphrase before starting the server.";
            return;
        }
    ...
    private async Task StopAsync(CancellationToken cancellationToken)
    {
        ...
        RemoteStatus = "Team server stopped. A new session passphrase is ready for the next start.";
    ...
    private async Task SetAuthenticationAsync(bool enabled, bool reauthenticate, CancellationToken cancellationToken)
    {
        if (enabled)
        {
            ...
            {
                RemoteStatus = "Enter a server passphrase before enabling authentication.";
                return;
            }

            await _workflow.SetAuthenticationAsync(true, reauthenticate, ServerPassphrase, cancellationToken);
            RequirePassphrase = true;
            RemoteStatus = reauthenticate
                ? "Authentication enabled. Connected observers were asked to re-authenticate."
                : "Authentication enabled. Existing observers remain connected; new observers must authenticate.";
        }
        else
        {
            await _workflow.SetAuthenticationAsync(false, false, cancellationToken: cancellationToken);
            RequirePassphrase = false;
            RemoteStatus = "Authentication disabled. New observers may connect without a passphrase.";
        }
        Refresh();
    }
    ...
    private async Task ProbeRemoteAsync(CancellationToken cancellationToken)
    {
        try
        {
            RemoteStatus = "Probing remote Robot Command...";
            ...
            RemoteStatus = $"Found {probe.ServerInfo.DisplayName} · {probe.ServerInfo.ApiVersion}. Enter its session passphrase, then select Connect.";
        }
    ...
    private async Task ObserveRemoteAsync(CancellationToken cancellationToken)
    {
        try
        {
            RemoteStatus = "Connecting securely and requesting host approval...";
            var probe = await _observers.ProbeAsync(RemoteEndpoint, cancellationToken);
            ...
            {
                RemoteStatus = "This server requires its session passphrase. Enter it, then select Connect.";
                return;
            }
            ...
            RemoteStatus = "Remote Robot Command is being mirrored read-only.";
        }
        catch (Exception ex) { RemoteStatus = ex.Message; }
    }
    ...
    private void CreatePairing()
    {
        try
        {
            ...
            RemoteStatus = "Pairing QR code is ready. It contains this session's endpoint and passphrase; host approval is still required.";
    ...
    private void RefreshLists()
    {
        ...
        if (authenticationRequired is not null && !RemoteRequiresPassphrase)
        {
            ...
            RemoteStatus = "The remote Robot Command requires a new session passphrase. Enter it, then select Connect.";
        }
        NearbyServers.Clear();
