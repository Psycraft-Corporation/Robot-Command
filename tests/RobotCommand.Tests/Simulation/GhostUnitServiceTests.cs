...
public sealed class GhostUnitServiceTests
{
    ...
    [Fact]
    public void BatchSpawnLayout_LeavesSingleGhostAtCenterAndSpacesBatches()
    {
        var center = new MapViewportSnapshot(-79.38, 43.65, 1, 0);

        var single = GhostSpawnLayout.ForBatch(center, 0, 1);
        var batch = Enumerable.Range(0, 4)
            .Select(index => GhostSpawnLayout.ForBatch(center, index, 4))
            .Cast<MapViewportSnapshot>()
            .ToArray();

        Assert.Equal(center, single);
        Assert.Equal(4, batch.Select(viewport => (viewport.LatitudeDegrees, viewport.LongitudeDegrees)).Distinct().Count());

        var zoomedOut = Enumerable.Range(0, 4)
            .Select(index => GhostSpawnLayout.ForBatch(center with { Resolution = 10 }, index, 4))
            .Cast<MapViewportSnapshot>()
            .ToArray();
        var closeLatitudeOffset = Math.Abs(batch[0].LatitudeDegrees - center.LatitudeDegrees);
        var distantLatitudeOffset = Math.Abs(zoomedOut[0].LatitudeDegrees - center.LatitudeDegrees);

        Assert.InRange(closeLatitudeOffset, 0.0001, 0.001);
        Assert.InRange(distantLatitudeOffset / closeLatitudeOffset, 9.5, 10.5);

        var worldScale = GhostSpawnLayout.ForBatch(center with { Resolution = 1_000 }, 0, 4);
        Assert.NotNull(worldScale);
        Assert.InRange(
            Math.Abs(worldScale.LatitudeDegrees - center.LatitudeDegrees) / closeLatitudeOffset,
            999.5,
            1_000.5);
    }
    ...
    public async Task TakeoffAndHeading_AreSimulatedAtRuntime()
    {
        ...
        // The simulation advances at 60 Hz, while telemetry publication and CI
        // scheduling are independent. Wait for the authoritative state instead
        // of assuming that a fixed wall-clock delay contains the full turn.
        for (var attempt = 0; attempt < 100; attempt++)
        {
            telemetry = stores.Telemetry.Items.ToArray().Single(item => item.VehicleId == ghost.Id);
            if (Math.Abs(telemetry.HeadingDegrees.GetValueOrDefault() - 180) <= 1)
            {
                break;
            }
