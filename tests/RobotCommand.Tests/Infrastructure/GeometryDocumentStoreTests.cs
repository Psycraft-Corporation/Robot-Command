...
public sealed class GeometryDocumentStoreTests
{
    ...
    public void Reload_RemovesAbandonedEmptyLegacyLocalDrafts()
    {
        ...
        try
        {
            ...
            File.WriteAllText(path, GeometryDocumentCodec.Serialize(legacy));
    ...
    public async Task Import_DoesNotOverwriteUnlessExplicitlyAllowed()
    {
        ...
        try
        {
            ...
            await File.WriteAllTextAsync(
                importPath,
                GeometryDocumentCodec.Serialize(codec.PrepareForSave(Route())));
            var store = new GeometryDocumentStore(root);
            ...
            await File.WriteAllTextAsync(
                importPath,
                GeometryDocumentCodec.Serialize(codec.PrepareForSave(replacement)));
    ...
    public async Task Import_RejectsSymbolicLinkWhenPlatformSupportsIt()
    {
        ...
        try
        {
            ...
            await File.WriteAllTextAsync(realPath, GeometryDocumentCodec.Serialize(codec.PrepareForSave(Route())));
            try
            {
