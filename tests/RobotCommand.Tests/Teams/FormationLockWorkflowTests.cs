...
public sealed class FormationLockWorkflowTests
{
    ...
    public async Task MixedGhostAndPx4Lock_UsesPerMemberExecutors_AndReleasesPx4ToHold()
    {
        ...
        await EventuallyAsync(() => px4.Updated.Count > 0, 5000);
    ...
    private sealed class RecordingPx4FormationExecutor : IFormationLockExecutor
    {
        ...
        public List<FormationMemberTarget> Begun { get; } = [];
        public List<FormationMemberTarget> Updated { get; } = [];
    ...
    private sealed class LiveUnits(LiveGhostStores stores) : IUnitObservationWorkflow
    {
        ...
        private UnitObservationSnapshot Project(VehicleRecord vehicle)
        {
            ...
            var number = vehicle.Id.StartsWith("ghost-", StringComparison.Ordinal)
                ...
                : vehicle.Id;
            if (!stores.Telemetry.TryGet($"ghost-telemetry-{number}", out var telemetry) || telemetry is null)
                throw new InvalidOperationException($"Telemetry for '{vehicle.Id}' was not available.");
            return Ghost(vehicle.Id, telemetry.LatitudeDegrees!.Value, telemetry.LongitudeDegrees!.Value, telemetry.AltitudeAglMetres!.Value)
                with
            {
                Telemetry = Ghost(vehicle.Id, telemetry.LatitudeDegrees!.Value, telemetry.LongitudeDegrees!.Value, telemetry.AltitudeAglMetres!.Value).Telemetry with
                {
                    Armed = telemetry.Armed,
                    LandedState = telemetry.LandedState,
                    Mode = telemetry.AdapterState,
                    AltitudeAglMetres = telemetry.AltitudeAglMetres,
