...
public sealed class ArduPilotVehicleDiagnosticsProviderTests
{
    ...
    [Fact]
    public void PreArmTextRemainsActiveUntilArduPilotReportsResolvingEvidence()
    {
        var now = DateTimeOffset.UtcNow;
        var snapshot = _provider.Build(Evidence(active: new Dictionary<string, DateTimeOffset>
        {
        ...
        Assert.Contains(snapshot.Blockers, item => item.Detail == "PreArm: GPS not healthy");
        Assert.Contains(snapshot.Checks, item => item.Code.Contains("STATUSTEXT", StringComparison.Ordinal));
    }
