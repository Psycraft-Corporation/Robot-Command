...
public sealed class MavlinkConnectionTests
{
    ...
    [Fact]
    public async Task ArduPilotReceiverBlockerDoesNotClearUntilReceiverEvidenceIsValid()
    {
        var transport = new FakeTransport();
        var connection = CreateConnection(
            transport,
        ...
        {
            transport.Emit(Heartbeat(1, autopilot: MavlinkValues.MavAutopilotArduPilot));
            transport.Emit(StatusText(1, "PreArm: Radio failsafe on"));
        };

        await connection.ConnectAsync(ConnectionCredentials.Empty, false, TestContext.Current.CancellationToken);

        transport.Emit(RcChannels(1, 0, ushort.MaxValue));
        Assert.Contains(connection.LiveSnapshot.VehicleDiagnostics,
            diagnostics => diagnostics.Blockers.Any(item => item.Detail == "PreArm: Radio failsafe on"));

        transport.Emit(RcChannels(1, 80, 1500, 1500));
        Assert.DoesNotContain(connection.LiveSnapshot.VehicleDiagnostics,
            diagnostics => diagnostics.Blockers.Any(item => item.Detail == "PreArm: Radio failsafe on"));
    ...
    [Fact]
    public async Task ArduPilotReceiverBlockerDoesNotClearWhenChannelRssiIsUnknown()
    {
        var transport = new FakeTransport();
        var connection = CreateConnection(
            transport,
        ...
        transport.Emit(RcChannels(1, 0, 1500, 1500));
    ...
    public async Task ArduPilotFailedArmAckUsesFreshPreArmStatusTextInsteadOfResultParam2()
    {
        ...
        transport.OnOpen = () => transport.Emit(Heartbeat(
            1,
            autopilot: MavlinkValues.MavAutopilotArduPilot,
            customMode: MavlinkValues.ArduPilotGuidedCustomMode));
        transport.OnSend = payload =>
        {
            if (payload is [2])
            {
    ...
    public async Task ArduPilotFailedArmAckWithoutStatusTextIncludesModeContext()
    {
        ...
        transport.OnSend = payload =>
        {
            if (payload is [5] && transport.Codec.LastCustomMode == MavlinkValues.ArduPilotGuidedCustomMode)
            {
                transport.Emit(Heartbeat(
                    1,
                    autopilot: MavlinkValues.MavAutopilotArduPilot,
        ...
        Assert.Contains("No vehicle status reason was reported", result.Message, StringComparison.Ordinal);
        Assert.DoesNotContain("Failed (0)", result.Message, StringComparison.OrdinalIgnoreCase);
        Assert.Contains(connection.LiveSnapshot.VehicleDiagnostics,
            diagnostics => diagnostics.OverallStatus == VehicleDiagnosticStatus.Blocked);
        await connection.DisposeAsync();
    }

    [Fact]
    public async Task ArduPilotArmFromLandedOrStabilizeModeEntersGuidedBeforeArm()
    {
        var transport = new FakeTransport();
        var connection = CreateConnection(
            transport,
        ...
        var guidedConfirmed = false;
        transport.OnSend = payload =>
        {
            if (payload is [5] && transport.Codec.LastCustomMode == MavlinkValues.ArduPilotGuidedCustomMode)
            {
                guidedConfirmed = true;
                transport.Emit(Heartbeat(
                    1,
                    autopilot: MavlinkValues.MavAutopilotArduPilot,
                    customMode: MavlinkValues.ArduPilotGuidedCustomMode));
            }
            else if (payload is [2] && guidedConfirmed)
            {
        ...
        Assert.True(modeIndex >= 0, "The Arm request did not enter ArduPilot Guided mode.");
        Assert.True(armIndex > modeIndex, "ArduPilot Arm was sent before Guided mode confirmation.");
        Assert.Equal(MavlinkValues.ArduPilotGuidedCustomMode, transport.Codec.LastCustomMode);
        await connection.DisposeAsync();
    }
    ...
    private static MavlinkPacket RcChannels(byte systemId, byte rssi, ushort channel1, ushort? channel2 = null)
        => new(
            2,
            4,
            systemId,
            MavlinkValues.MavCompIdAutopilot1,
            MavlinkMessageIds.RcChannels,
            new Dictionary<string, object>
            {
                ["rssi"] = rssi,
                ["chan1_raw"] = channel1,
                ["chan2_raw"] = channel2 ?? ushort.MaxValue
            },
            DateTimeOffset.UtcNow);
