...
public sealed class ArduPilotMavlinkAutopilotAdapterTests
{
    ...
    [Fact]
    public void ConfirmedAirborneDisarm_UsesForceDisarmParameter()
    {
        var command = _adapter.BuildCommand(
            Request(
        ...
        Assert.Equal(MavlinkCommandIds.ComponentArmDisarm, command.CommandId);
        Assert.Equal(21196f, command.Parameters[1]);
    }

    [Fact]
    public void NormalDisarm_DoesNotUseForceDisarmParameter()
    {
        var command = _adapter.BuildCommand(
            Request(OperatorCommandKind.Disarm, OperatorCommandParameters.None),
            Telemetry());

        Assert.Equal(0f, command.Parameters[1]);
    }
    ...
    private static OperatorCommandRequest Request(
        ...
        => new("command", "correlation", "idempotency", command,
            ...
            "test", emergency, DateTimeOffset.UtcNow, Parameters: parameters);
