...
public sealed class AppConfigurationTests
{
    ...
    public void Load_ReadsSupportedLanguageAndFallsBackForUnknownLanguage()
    {
        ...
        var japaneseDirectory = CreateTempDirectory();
        File.WriteAllText(Path.Combine(japaneseDirectory, "appsettings.local.json"),
            "{ \"ui\": { \"language\": \"ja\" } }");

        Assert.Equal("ja", AppConfiguration.Load(japaneseDirectory).Ui.Language);
