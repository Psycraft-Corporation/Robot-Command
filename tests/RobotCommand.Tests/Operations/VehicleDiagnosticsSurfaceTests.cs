...
public sealed class VehicleDiagnosticsSurfaceTests
{
    ...
    public void OperateUnitSurface_UsesNormalizedDiagnosticsWithoutLegacyReadinessPanel()
    {
        ...
        Assert.DoesNotContain("IsMavlinkParameterPanel", connections);
        Assert.DoesNotContain("ParameterPanelTitle", connections);
        Assert.DoesNotContain("ParameterBackendUnit", connections);
        ...
        var units = File.ReadAllText(Path.Combine(
            FindRepositoryRoot(), "src", "app", "RobotCommand", "Views", "Workspaces", "UnitsLibraryView.axaml"));
        Assert.Contains("IsMavlinkParameterPanel", units);
        Assert.Contains("ParameterTargets", units);
        Assert.Contains("DownloadParametersCommand", units);
    }
