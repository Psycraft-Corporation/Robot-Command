...
public sealed class ManualControlSurfaceTests
{
    ...
    [Fact]
    public void ManualControlWorkspaceShowsPendingControllerTimeout()
    {
        var root = FindRepositoryRoot();
        var view = File.ReadAllText(Path.Combine(root, "src", "app", "RobotCommand", "Views", "Workspaces", "ManualControlView.axaml"));

        Assert.Contains("CONTROLLER COMMAND PENDING", view, StringComparison.Ordinal);
        Assert.Contains("PendingButtonExpiry", view, StringComparison.Ordinal);
